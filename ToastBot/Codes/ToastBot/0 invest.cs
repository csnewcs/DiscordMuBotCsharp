    ...
    class invest
    {
        public async Task xnwk(SocketMessage message)
        {
            ...
            if (int.Parse(turn) >= bread)
            {
                ...
                if (ran == 100) { result *= 100; send = playername + "! 극악의 확률을 뚫었다뮤! 보상으로 100배로 돌려줄게뮤! " + result + "개 획득"; }
                else if (ran == 99 || ran == 98) { result /= 200; send = playername + "! 극악의 확률을 뚫었다뮤! 하지만 안좋은쪽이다뮤ㅠ" + result + "개 획득"; }
                ...
                else if (ran >= 10) { result /= 2; send = playername + "님은 " + bread + "개의 빵을 투자해서 실패해 " + result + "개의 빵을 받았다뮤!"; }
                else { result /= 4; send = playername + "님은 " + bread + "개의 빵을 투자해서 폭망해 " + result + "개의 빵을 받았다뮤!"; }
